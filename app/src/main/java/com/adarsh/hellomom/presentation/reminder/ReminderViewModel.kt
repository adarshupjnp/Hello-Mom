...
class ReminderViewModel @Inject constructor(
    ...
    val displayedReminders: StateFlow<List<ReminderEntity>> =
        combine(_familyReminders, reminders, _selectedDate) { liveList, roomList, date ->
            ...
            // Merge the live Firestore list + the Room list, then collapse duplicates so each
            // reminder shows exactly once — even after a snooze diverges its time. Auto-reminders
            // dedupe by title+date, custom by id; freshest copy (latest status) wins. See ReminderDedup.
            val merged = ReminderDedup.dedupe(liveList + roomList)
            ...
            if (date == null) {
                // "All" view: newest day first (today, then yesterday, then 22nd …), and within
                // each day in morning → night order so a day reads as a natural timeline.
                filtered.sortedWith(
                    compareByDescending<ReminderEntity> { startOfDay(it.time) }.thenBy { it.time }
                )
            } else {
                // A single selected day stays in morning → night order (unchanged).
    ...
    private fun initializeDailyReminders() {
        viewModelScope.launch {
            ...
            // Generation, idempotency and alarm scheduling all live in the repository so the exact
            // same logic runs here (instant feedback when the owner opens the screen, works offline),
            // from the background SyncWorker, and from the midnight day-change rollover. Callers
            // already gate this on owner — family members are viewers and never generate.
            runCatching { reminderRepository.ensureDailyReminders(user.userId, user.fullName) }
                .onFailure { SyncLogger.warn("initializeDailyReminders failed", it) }
        }
    }
